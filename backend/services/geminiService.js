 ...
 * @returns {Object} 请求体
 */
const buildRequestPayload = (prompt, imageUrls, aspectRatio, temperature) => {
...
export const generateImage = async ({
  ...
  try {
    // 构建请求体
    const payload = buildRequestPayload(prompt, image_urls, aspectRatio, temperature)
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        errorMessage = '请检查 API Key 是否有效，以及是否在 Google AI Studio 启用了计费'
